...
public class EmployeeRemove extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_employee_remove);
    ...
    public void RemoveEmployee(View view){
        EditText id;
        id = findViewById(R.id.edt_empId);

        if(id.getText().toString().isEmpty()){
            Toast.makeText(getApplicationContext(), "Error: Id MUST be entered", Toast.LENGTH_LONG).show();
        }
        else{
            db.collection("Employee").document(id.getText().toString()).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
